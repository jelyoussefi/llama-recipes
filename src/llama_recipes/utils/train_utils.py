...
def trace_handler(prof):
    output = prof.key_averages().table(sort_by="self_xpu_time_total", row_limit=10)
    print(output)
    prof.export_chrome_trace("trace_" + str(prof.step_num) + ".json")
    torch.save(prof.key_averages().table(sort_by="self_xpu_time_total"), "profile_" + str(prof.step_num) + ".pt")
    torch.save(prof.key_averages(group_by_input_shape=True).table(sort_by="self_xpu_time_total", row_limit=-1), "profile_ops_" + str(prof.step_num) + ".pt")
...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    if train_config.enable_profiler:
        wait, warmup, active, repeat = 1, 1, 1, 0
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(train_dataloader):
                with profiler:    
                    if step == (1 + train_config.max_steps):
                        break
                    step_start_time = time.perf_counter()

                    # env for unitrace conditional collection, only only step == 3
        ...
        print("Rank: ", rank, ", Step Times: ", steps_times)
...
def get_policies(cfg, rank):
    ...
    # )
    verify_bfloat_support = is_xpu_available()
