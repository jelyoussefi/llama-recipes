...
def get_llama_attention_patch_fn(patch_fn_name: str = 'torch') -> Callable:
    ...
    elif patch_fn_name == 'ipex':
        return llama_attention_patch_torch #temp WA
    else:
        raise ValueError(
            f'Unrecognized llama attention patch function: {patch_fn_name}')

def ipex_custom_flash_attn_fn(
    query,
    key,
    value,
    n_heads,
    ...
    q = rearrange(query, 'b s (h d) -> b h s d', h=n_heads) # [b=4, h=12, s=2048, d=64]
    kv_n_heads = 1 if multiquery else n_heads
    k = rearrange(key, 'b s (h d) -> b h s d', h=kv_n_heads) # custom flash attention
    ...
    print("ipex FA q", q.shape)
    print("ipex FA k", k.shape)
    print("ipex FA v", v.shape)
    
    ### custom flash attention 
    context_layer = torch.xpu.FlashScaledAttn(q, k, v, softmax_scale, is_casual=True)

    out = rearrange(context_layer, 'b s h d -> b s (h d)')
...
def llama_attention_patch_torch(
    ...
    # IPEX Flash Attention does not take softmax_scale = None
    softmax_scale = 1 / math.sqrt(self.hidden_size / self.num_heads)
    ...
    attn_output = rearrange(context_layer, 'b s h d -> b s (h d)')
    attn_weights = None
